...
-- CREATE INDEX IF NOT EXISTS idx_orderprocessing_status ON orderprocessings (status);

-- Created date (pagination/sorting by date)
-- CREATE INDEX IF NOT EXISTS idx_orderprocessing_created_at ON orderprocessings (created_at DESC);

-- Composite: status + created_at (covers most list queries)
-- CREATE INDEX IF NOT EXISTS idx_orderprocessing_status_created
--     ON orderprocessings (status, created_at DESC);

-- Foreign key index (prevents full-table scan on joins)
-- CREATE INDEX IF NOT EXISTS idx_orderprocessing_user_id ON orderprocessings (user_id);
